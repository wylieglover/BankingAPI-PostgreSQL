...
class TransactionModel {
    constructor(prisma) {
        ...
        this.getAverageTransactionAmount = async () => {
        ...
        this.getTransactionTypeDistribution = async () => {
        ...
        this.getTransactionVolumeByDay = async () => {
        ...
        this.getAllTransactions = async (params) => {
            try {
        ...
        this.updateTransaction = async (updateData) => {
            try {
                const existingTransaction = await this.prisma.transactions.findUnique({
                    where: { transaction_id: updateData.transactionId },
                });
                if (!existingTransaction) {
                    throw new Error(`Transaction with ID ${updateData.transactionId} does not exist.`);
                }
                return await this.prisma.$transaction(async (prisma) => {
                    if (updateData.type || updateData.amount) {
                        const { account_id, type: oldType, amount: oldAmount } = existingTransaction;
                        await prisma.accounts.update({
                            where: { account_id },
                            data: {
                                balance: {
                        ...
                        if (updateData.amount && updateData.type) {
                            await prisma.accounts.update({
                                ...
                                data: {
                                    balance: {
                                        [updateData.type === 'deposit' ? 'increment' : 'decrement']: updateData.amount,
                                    },
                                },
                            });
                        }
        ...
        this.deleteTransaction = async (transactionId) => {
            try {
                const transaction = await this.prisma.transactions.findUnique({
                    where: { transaction_id: transactionId },
                });
                ...
                return await this.prisma.$transaction(async (prisma) => {
                    await prisma.accounts.update({
                        ...
                        data: {
                            balance: {
                                [transaction.type === 'deposit' ? 'decrement' : 'increment']: transaction.amount,
                            },
                        },
                    });
